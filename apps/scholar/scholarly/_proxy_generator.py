...
class ProxyGenerator(object):
    ...
    def _new_session(self, **kwargs):
        ...
        # Pop proxies if present in init_kwargs to avoid httpx Client crash
        ...
        if self._proxy_works:
            # Clean and map to httpx 'proxy' argument
            proxy_url = proxies.get("http://") or proxies.get("http") or proxies.get("https://") or proxies.get("https")
            if proxy_url:
