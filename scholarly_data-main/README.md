...
# 🎓 Scholar Matcher Desktop App

Ứng dụng Desktop giúp cào thông tin tác giả và danh sách bài báo khoa học từ Google Scholar, tự động đối chiếu phân hạng tạp chí quốc tế (**SJR Q1-Q4**) và hội nghị (**CORE A*-C**, **CCF A-C**, **ABDC**).

## 📸 Giao diện ứng dụng (Screenshot)

![Giao diện chính](images/image.png)

## 🚀 Hướng dẫn chạy ứng dụng

Di chuyển vào thư mục dự án và chạy:
```bash
python3 scholar_scraper_gui.py
```

## 📑 Các tính năng chính

### 1. Chức năng 1: Cào toàn bộ dữ liệu thô (Sidebar Trái)
*   Nhập link hồ sơ Google Scholar hoặc gõ trực tiếp tên tác giả.
*   Cào nhanh (**Fast Mode**) không lo chặn IP, hoặc cào chi tiết (**Deep Mode**).
*   **Bật/Tắt đối chiếu phân hạng:** Tự động đối chiếu xếp hạng đa kênh (`SJR`, `CORE`, `CCF`, `ABDC`) hoặc tắt đi chỉ xem thông tin thô.
*   **Xuất dữ liệu JSON:** Xem trước cấu trúc JSON ngay trên phần mềm và tải tệp tin `.json` về máy.

### 2. Chức năng 2: Bộ lọc & Thao tác kết quả (Phần trên bảng hiển thị)
*   **Lọc theo từ khóa:** Lọc bài báo theo tên bài, tên tác giả phụ hoặc tạp chí.
*   **Lọc khoảng năm:** Lọc bài viết xuất bản từ năm... đến năm...
*   **Lọc theo hạng:** Lọc lấy các bài hạng Q1, Q2, A*, A, B... của bất cứ hệ thống nào.
*   **Giới hạn số lượng:** Lọc lấy Top 10, 25, 50, 100 bài báo có số trích dẫn cao nhất.
*   **Xuất báo cáo:** Xuất danh sách bài báo đã lọc ra file Excel (`.xlsx`) hoặc CSV (`.csv`) chỉ bằng một cú click.

---

# scholarly (Lõi thư viện)
